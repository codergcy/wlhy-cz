	...
	<view class="zai-box">
		<scroll-view scroll-y class="page">
			<view class="text-center" :style="[{animation: 'show ' + 0.4+ 's 1'}]">
				<image src="https://wlhy-1310215349.cos.ap-shanghai.myqcloud.com/logo.png" mode='aspectFit'
					class="zai-logo "></image>
				<view class="zai-title text-shadow ">驼韵运营 </view>
			</view>
	...
	export default {
		...
		methods: {
			...
			onLogin: function() {
				...
				// 密码转换成base64
				var encryptionStr = new Buffer(this.password).toString('base64');
				// 密码登录query
				let loginParams = {
					remember_me: '',
					isPassword: 1,
			...
			onSMSSend() {
				...
				this.$http.post("/sms/send/verifyCode", smsParams).then(res => {
					if (res.data.success) {
